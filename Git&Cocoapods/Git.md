...
1. 里面的commit id是通过SHA1计算的16进制的数
2. `git log --graph`可以查看改动的图形状态

### 查看文件的修改

`git status`命令用于显示工作目录和暂存区的状态。使用此命令能看到那些修改被暂存到了, 哪些没有, 哪些文件没有被Git tracked到。
> 通过`git diff`可以查看更改的内容 **会使用文件补丁的格式显示具体添加和删除的行**
...
3. 用命令`git reset HEAD <file>`可以把暂存区的修改撤销掉（unstage），重新放回工作区 后面的是版本号
   ...
   2. `git reset --merge`撤销上次的merge

### 更新本地仓库

`git pull`更新本地仓库到最新
`git fetch orgin`一次性将所有的远程分支全在本地更新
...
1. 每次提交master都向前移动，**master分支**也越来越长
2. 创建新的分支，例如dev。
...
1. 会将当前 feature 分支的所有的commit取消掉
