...
功能：如果不在蓝牙道钉的区域范围内，加大还车的难度，以及一定的调度费。
我的指责：我的第一个简单的需求，主要负责UI的实现。由于两个页面有比较多的重复内容，通过将一个页面复用来完成
        1. 整体是通过MVP的模式来实现
        2. 整个页面是通过一个UITableView来完成，这样适配起来会比较方便
        3. sectionHeader、sectionFooter以及cell三部分
           1. header和footer的内容仅仅是文案的不同（我是通过一个枚举来区分）
        ...
        4. 更多的是通过Masonry来实现（Masonry来设定）

## 骑行结束（9月预热弹窗）（即将上线）

功能：在9月份有一个营销活动的预热弹窗
我的指责：整个开发流程的完成
        1. 在骑行结束不仅仅只有一个弹窗，这个在别人的代码基础上添加一个更高优先级的弹窗
        2. 这个弹窗出现的时机，图片，跳转链接都是服务器配置的
           1. 需要向服务器请求弹窗数据，但是这个请求又是异步的
           2. 我是通过回调的block来调用弹窗，将同事的代码通过也通过block包起来
              1. 根据数据选择执行情况
        3. 这个弹窗就是一个UIViewController
