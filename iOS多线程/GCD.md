...
3. 同步函数+主对列 = 死锁
   ...
   2. 应该是一种相互等待的关系，当前主线程正在执行sync，而sync要等待block执行完，但是现在插入的是mainqueue，只能等待当前的主线程执行完当前任务才能执行这个block
4. 异步函数+主对列 = 不会新建线程，所有任务都会在主线程中串行执行
...
可以通过设置onceToken为0来释放，但是`static dispatch_once_t onceToken`要放在外部
